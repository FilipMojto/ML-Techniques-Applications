...
async def get_all_users():
    ...
    for user in users:
        ...
        user_profiles.append(UserProfile(username=username, liked_movies=liked_movies))
...
@app.get("/user/{username}/movies", response_model=Profile)
async def get_movie_subset(
    username: str,
    start: int = Query(0, ge=0, description="Zero-based index to start the slice from"),
    limit: int = Query(10, ge=1, description="Maximum number of movies to return"),
):
    """Return a page of liked movies for the given user, starting at `start` with length `limit`."""
    conn = get_db_connection()
    cursor = conn.cursor()
    ...
    # Get the user's full movie list
    cursor.execute("SELECT movie_title FROM user_movie_list WHERE user_id = ?", (user_id,))
    liked_movies = [r["movie_title"] for r in cursor.fetchall()]
    conn.close()

    # Validate range
    if start >= len(liked_movies):
        raise HTTPException(status_code=400, detail="`start` index out of range")

    # Slice
    subset = liked_movies[start : start + limit]
    return Profile(liked_movies=subset)
