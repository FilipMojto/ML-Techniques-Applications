...
from isa_project_1.config import DOWLOADED_DATASET_DIR, INTERIM_DATASET_DIR, INTERIM_DATASETS  # Custom config module
...
def main(
    ...
    """
    for dataset, output_path in INTERIM_DATASETS.items():
        ...
        for i in tqdm(range(10), total=10):  # For each iteration, display progress
            if i == 0:
                ...
                if not Path(INTERIM_DATASET_DIR).exists():
                    logger.info("Output directory doesn't exist. Creating new...")
                    Path(INTERIM_DATASET_DIR).mkdir(parents=True, exist_ok=True)  # Create the directory if it doesn't exist
